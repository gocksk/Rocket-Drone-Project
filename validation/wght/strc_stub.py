...
[왜 선형 스텁이 기본인가]
멱함수 m_str = a·MTOW^b 는 b != 1 일 때 구조 비중이 반복 중에 변한다. 그러면 Ŝ 가
상수가 아니라서 '경계가 Ŝ=1 이다'라는 이론값과 대조할 수 없다.
선형 m_str = S·MTOW 는 S 가 기울기 그 자체이고 고정점이 해석적으로 나온다:
...
def _out(m_str, l_body=1.0, x_cg_str=0.5, r_body=0.05):
    """스텁의 StrcOut. 구조를 길이 l_body 인 균일 봉으로 근사한다.

    실제 STRC 가 오면 통째로 대체된다 — 여기서 중요한 건 값의 정확도가 아니라
    m_str 이 MTOW 에 어떻게 반응하는가다. breakdown_str[0] 은 WGHT §5 가 동체 쉘로
    쓰므로(길이항 J) 최소 한 항목은 있어야 한다.
    """
    bd = [MassItem("shell", m_str, x_cg_str, r_body)]
    return StrcOut(m_str=m_str, m_print=m_str * 1.05, m_fill=m_str * 0.1,
                   w_fill=0.01, breakdown_str=bd, g10=1.0)


def make_linear_strc(S, **kw):
    """m_str = S · MTOW.  고정점 MTOW* = m_fixed/(1-S) 가 해석적으로 나온다.
...
def make_quantized_strc(S, quantum, **kw):
    """m_str = quantum · round(S·MTOW / quantum).  계단형 출력.
...
def make_noisy_strc(base_fn, rel_noise, seed=0):
    ...
    def strc_of(MTOW):
        ...
        return _out(st.m_str * (1.0 + rel_noise * h))
    return strc_of
