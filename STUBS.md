...
### `modules/strc.py` — 구현 완료, 슬라이서 계수 TBD (P6)
기하 기반 질량(쉘·핀·격벽) + 응력 검산. 벽두께·인필은 **프린트 하한과 응력 요구의
최대값**이다 (docs §11-33).

| 항목 | 상태 | 남은 것 |
|---|---|---|
| 질량 모형 | 구현 | `k_sl_*`(슬라이서 할증) `rho_mat` `t_0` `k_t` TBD |
| 응력 검산 (g5) | 구현 | `sigma_allow` `SF` `alpha_lim` TBD |
| 벽두께 사이징 | 구현 | STRC 가 `t_wall` 을 내는데 **GEOM 이 같은 식을 복제**하고 있다 (docs §11-27). 한쪽만 고치면 조용히 어긋난다 |

⚠ **구조가 응력이 아니라 프린트 가능성으로 정해진다.** MTOW 0.7 kg 에서 응력이
요구하는 벽두께는 0.02 mm 인데 프린트 하한이 1.80 mm — **88배 과설계**다.
응력이 두께를 정하기 시작하는 것은 MTOW 60 kg 부근이다. 그 결과:
- `W_str` 이 이 설계공간에서 MTOW 에 **거의 반응하지 않는다** → **Ŝ 가 0.319 → 0.0298**
  로 떨어졌다. 무게 스노우볼은 모터·배터리가 만든다
- g5 가 +0.97 로 사실상 죽어 있다 (응력이 허용의 1.2%)
- **W_str = 594 g 이 MTOW 1.076 kg 의 55% 다.** PLA 1.8 mm 벽이 이 크기에서
  지배적으로 무겁다는 뜻이다. 재료·벽두께 재검토가 필요하다 [확정 필요]
