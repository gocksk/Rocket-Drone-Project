...
기준: **ICD0-008** (원문 사본: `docs/ICD0-008.md`) · 단계: **P5.6 완료 시점**
...
- **설계점당 실행 시간 5.06 s.** P5.6 에서 §4.5 U_eval 순환을 `resp_of` 안에서
  ...
  P7 게이트의 실측 대상.
- **`thr_dash` = 0.85 가 근거 없는 자리표시다.** kv 를 정하는 기준인데(docs §11-31),
  값의 근거는 조종 여유 = g9 다. **P6 에서 STAB 이 조종 모멘트를 계산하면 교차검증**해야
  한다. 지금 값에서 C3 설계점 간 퍼짐이 0.05, 0.75 로 낮추면 0.16 이다.
