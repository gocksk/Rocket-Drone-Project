# 미구현 목록 (STUBS)

기준: **ICD0-008** (원문 사본: `docs/ICD0-008.md`) · 단계: **P7 게이트 통과 + DOE 배관 부설**

이 문서는 "아직 진짜가 아닌 것"의 유일한 목록이다. 코드에서 `[스텁]` 또는 `# TBD` 를
지울 때 여기서도 지운다. 그럴듯한 물리값을 지어내 채우는 것이 이 프로젝트에서 가장
비싼 실수이므로, 값이 없으면 없다고 적는다.

표기
- **스텁** — 함수는 있고 시그니처는 맞지만 반환값이 자리표시 숫자다. 물리적 근거 없음.
- **TBD** — 상수 값이 미확정. 붙은 숫자는 임시이며 근거 없음.
- **정의식** — 물리 모델이 아니라 정의라서 구현이 끝났으나, 쓰는 상수가 TBD 인 경우.

---

## 1. 모듈별 미구현

### `modules/atm.py` — **스텁 없음**
`run(h)` 는 실제 구현이며 검산까지 통과한다 (`python -m modules.atm`).
h=0 에서 ρ=1.2250 · a=340.29 · μ=1.789e-5. 기존 `common/atm.py` 를 ICD0-008
시그니처로 옮긴 것이다.

### `modules/geom.py` — 전부 구현 (P2 `hull` · P5.5 배치)
| 함수 | 상태 | 남은 것 |
|---|---|---|
| `hull(dv)` | 구현 | 손계산 대조 통과 |
| `wall_thickness` | 구현 | 개념상 STRC 소관이나 `d_internal` 이 ⓪에서 먼저 쓴다. **GEOM 이 단독 소유**하고 `HullOut.t_wall` 로 STRC 에 보낸다 (docs 11-41). 식을 여기 밖에서 다시 쓰지 않는다 |
| `box_from_volume` | 구현 | 내부 원에 **내접**하는 상자 = 기하학적 최대. 배선·완충재 몫이 없어 **g6 이 낙관적**이다 (충전율 계수 미정) |
| `pod(...)` | 구현 | 포드 기하. §11-2 포드 항력 배선이 이걸 쓴다 (P5.6) |
| `layout(...)` | 구현 | `LAYOUT_ORDER` 순서로 기수부터 적재 · `arm_rotor = r_body + f_mount·b_fin` |
| `check_fit(...)` | 구현 | g6(길이·단면 중 나쁜 쪽) · g7(로터간·동체간 중 나쁜 쪽) |

⚠ **적재 순서에 ESC 가 없다.** ICD 어디에도 ESC 질량·치수가 없다 (COST 에 단가만).
실제로는 동체 안에서 자리를 차지하므로 **g6 이 그만큼 낙관적**이다.

`hull` 은 ICD §5.1 대로 포드를 빼고 노즈+원통+핀만 낸다. 포드 치수는 모터 질량의
함수라 ⓪에서 알 수 없다. 형상 계수 `f_nose` `k_nose` `lam_fin` `tc_fin` `k_thk` 는 TBD.

### `modules/aero.py` — 구조 완료, 계수 TBD (P2 완료)
성분별 빌드업 · Prandtl-Glauert 압축성 보정 · Barrowman · 크로스플로 · 풍축 변환까지
표준식으로 구현했다 (`python -m modules.aero`). 반응 방향은 믿을 수 있고, **절대값은
아래 계수가 확정되기 전까지 보증되지 않는다.**

| 항목 | 상태 | 남은 것 |
|---|---|---|
| `F_drag(V, α)` · `CD0` | 구현 | 계수 `k_base` `k_int` `k_cal` TBD |
| `CL(V, α)` | 구현 | ICD 는 `CL(α)` — 축력항의 V 의존 때문에 넓혔다 (docs §11-3) |
| `CN_alpha` · `x_cp` | 구현 | `k_cp_nose` TBD |
| 크로스플로 | 구현 | `Cd_c` `eta_cf` `k_side` `k_finproj` TBD |
| 포드 항력 | **배선 완료** | 런처가 `pod_of(m_mot)` 클로저를 조립해 `size_motor`·`required_energy`·`achieved_range`·`evaluate` 에 전부 넘긴다. 순항 CD0 0.4599 → 0.4910 (+6.8%) |

### `modules/prop.py` — 전부 구현 (P3 맵·작동점 · P4 사이징 · P5.6 평가)
| 함수 | 상태 | 남은 것 |
|---|---|---|
| `U_ocv(SOC)` | 정의식 | 3점 선형. 상수 `U_cell_*` 가 TBD |
| `R_pack` · `U_eval` | 정의식 | `k_Rpack` TBD |
| `n_tip_limit` · `M_tip` | 구현 | — |
| `J_cruise` · `pd_prop_min` | 구현 | §2 하한 규칙(계수 1.25). DOE 범위 설정용 |
| `build_map(dv, air)` | **구현** | BEMT + Prandtl 팁손실. 블레이드 규약 7종이 TBD (docs §11-15). **앵커 보정 없음 → 호버 추력 과대평가** (§8 A-5) |
| `_trim` · `solve_point(…)` | **구현** | 트림 2×2 뉴턴 + rpm·kv 이분법. 243 케이스 100% 수렴 |
| `motor_elec(…)` | 정의식 | §4.3 수식은 확정 |
| `motor_regression(m_mot, kv)` | **1차 조사 완료** | 표본 13종 적합 (`data/`). R_mot R²=0.907 · I0 R²=0.752. **출처가 전부 2차라 팀 보충 필요.** 유효 범위 m 8.5–76 g · kv 1300–3800 밖은 외삽 |
| `size_motor(…)` | **구현** | m_mot 결정론적 이분법(20회) + THRM 직접 호출. `kv` 는 요구 최소 모터가 정하고 `k_mot` 은 그 kv 를 유지한 채 질량만 키운다 (docs §11-29) |
| `hover_wake` · `t_hover_worst` | 구현 | 착륙 호버(20 s)를 열 최악 조건으로 채택 |
| `evaluate(…)` | **구현** | V_max 격자훑기+이분법(§4.2 평형 실계산) · SPL 앵커 스케일링. **C3·C6 진짜 값** |

⚠ `build_map` 이 설계점당 약 **149 ms** 다. ⓪ 에서 가장 무겁고 DOE 규모의 지배 항목이 된다.

### `modules/thrm.py` — 구조 완료, 계수 TBD (P4 완료)
`motor_rise(…)` — 덩어리 열용량 1차 ODE + 표면적 환산 + 구간별 대류 상관식 구현
(`python -m modules.thrm`). ICD 의 설계상 주의 세 가지를 전부 반영했다.

검산: 34 g 모터 → 외경 27.9 mm (실제 2207 급 28 mm 와 일치) · 순항/호버 h·A 비 6.1배 ·
호버 시정수 91 s · **초기 온도를 외기로 두면 28.1 K 과소평가** (ICD 주의 2 의 크기)

| 항목 | 상태 | 남은 것 |
|---|---|---|
| `motor_geometry` | 구현 | `ar_mot` `rho_mot` TBD |
| `motor_rise` | 구현 | `c_mot` `T_limit` `k_air` TBD |
| 대류 상관식 | 구현 | **`Nu_C_*`·`Nu_m_*` 가 순항·호버 같은 값이다.** ICD 는 "구간별로 완전히 다르다" 고 했다. 호버는 프롭 후류라 상관식 형태부터 달라야 한다 |

### `modules/miss.py` — 구현 완료, 계수 TBD (P5 완료)
| 함수 | 상태 | 남은 것 |
|---|---|---|
| `segments(R_dash)` | 구현 | dash 소요 시간을 **요구 거리**에서 만든다 |
| `integrate(…)` | 구현 | 고정 스텝 · 팩 전압 강하 2차식 · 내부손실 계상 |
| `required_energy(…)` | 구현 | E_batt 이분법 3종(energy·sustain·power)의 최대값 |
| `achieved_range(…)` | 구현 | **같은 커널**을 거리에 대해 이분법 → C2 진짜 값 |

검증: 스텝 절반마다 변화율이 정확히 반씩(+0.083%→+0.041%→+0.028%) — 고정 스텝
1차 정확도의 이론값 · energy 활성일 때 `k_E=1.0` 에서 R_dash 오차 **−0.01%** ·
`k_E` 1.0→1.5 에서 R_dash 3000→4856 m · 순수성 확인

⚠ **활성조건이 지금 `sustain` 이다.** `c_rate_max`(90C)와 `k_Rpack`(0.010)이 서로
안 맞아서다 — 규정상 90C 를 허용하는데 그 전류에서 팩 전압이 무너진다.
`k_Rpack` 을 0.0005 로 낮추면 `energy` 로 넘어간다. 둘 중 하나가 틀렸다.

### `modules/strc.py` — **STRC 담당 코드 이식** (ICD0-007 → 008 어댑터)

본문은 STRC 담당이 쓴 코드다. 계산 로직·함수 구조·변수명을 그대로 두고 ICD0-008
인터페이스 변환만 파일 아래 `_GeomShim` · `run` 에 가뒀다. 원본 대비 손댄 곳은
**다섯 군데**이며 전부 `[이식 n/5]` 로 표시돼 있다.

| 항목 | 상태 | 남은 것 |
|---|---|---|
| 질량 모형 (쉘·핀 외피/코어·포드 쉘) | 구현 | `k_sl_*` `rho_mat` `t_0` `k_t` `w_line` `n_peri` `k_dens` TBD |
| 하중 케이스 3종 (추력 / 고속 off-design / 합성) | 구현 | `n_ref_load` `alpha_lim` TBD |
| 굽힘 + 전단 분리 검산 (g5) | 구현 | `sigma_cat` `k_layer` `k_tau` `k_w` `k_sec` `k_taper` TBD |
| 루트 보강 폭 `w_fill` 폐형해 | 구현 | 슬라이서 line width 로 **양자화**된다 — 아래 참조 |
| 벽두께 사이징 | 구현 | **해결.** `geom.wall_thickness` 단독 소유 → `HullOut.t_wall` 로 흐른다 (docs 11-41). 식이 한 곳뿐이다 |

⚠ **`w_fill` 양자화가 MTOW 수렴에 계단을 만든다.** `w_fill = ceil(w_req/w_line)·w_line`
이라 MTOW 가 올라가면 `W_str` 이 매끄럽게가 아니라 뛴다. 실측:

| | 값 |
|---|---|
| 계단 높이 (`W_str` 점프) | **743 mg** |
| 계단 간격 | MTOW 약 0.14 kg 마다 |
| WGHT 허용오차 (`eps_conv` 1e-3 × MTOW 1.2 kg) | **1200 mg** |
| 여유 | **1.6배뿐** |

지금은 수렴한다(`converged`, n_iter 12). 하지만 `eps_conv` 를 1e-4 로 조이거나
`w_line` 을 키우면 **limit_cycle 이 확실히 난다.** 그때 나오는 진동은 발산이 아니라
슬라이서 이산화이므로 WGHT 가 `limit_cycle` 로 분류하는 게 맞다 (ICD §5.1 STRC 경고). [확정 필요]

⚠ **여전히 프린트 가능성이 지배한다.** 다만 이식 전보다는 나아졌다:

| | 이식 전 (내 모델) | 이식 후 (담당 코드) |
|---|---|---|
| `W_str` | 659.0 g | **799.1 g** |
| 구성 | 쉘 473.5 · 핀 153.5 · 격벽 32.0 | 쉘 473.5 · 핀 248.0 · 포드 쉘 58.2 · 루트보강 19.4 |
| MTOW 반응 | **없음** (1.0→3.0 kg 에서 659.0 g 고정) | 797.3 → 808.3 g |
| g5 | +0.9675 고정 (사실상 죽은 조건) | +0.002 ~ +0.077 (**살아 있는 조건**) |

- 하중이 실제로 들어가는 자리(핀 루트)에만 MTOW 반응이 생겼다. 물리적으로 옳다.
- 그래도 반응은 작다 — 무게 스노우볼은 여전히 모터·배터리가 만든다.
- **`W_str` 이 MTOW 1.219 kg 의 66% 다.** 이식 전 55% 보다 더 나빠졌다.
  PLA 1.8 mm 벽 + 10.2 mm 두께 핀이 이 크기에서 지배적으로 무겁다.
  재료·벽두께 재검토가 필요하다 [확정 필요]

⚠ **이식으로 예제 설계점이 바뀌었다.** 늘어난 질량이 핀·포드(x≈0.593)에 붙어 x_cg 가
후퇴한다. 이전 점(`S_fin` 0.036 · `x_fin` 0.55)은 SM 0.917 → 0.839 로 떨어져 g8
불합격이 됐다. 구조 모델이 바뀐 결과지 이식 버그가 아니다.
`x_fin` 0.60 · `S_fin` 0.030 으로 옮겨 g1~g9 를 다시 전부 통과시켰다.

✅ **확정된 것 (docs 11-41~43).**

| 항목 | 결정 |
|---|---|
| 상수 충돌 11건 | **`constants.py` 값 채택.** `phi_0` 0.25 · `k_phi` 0.05 · `n_ref_load` 3.0 · `rho_mat` 1240 · `k_sl_shell` 1.15 · `k_sl_fin` 1.12 · `alpha_lim` 4° 등. 담당자 코드는 안 고쳤다 — 이름이 `constants.py` 에 있는 것만으로 우리 값이 이긴다 |
| `CN_alpha_fin` 정규화 | **AERO 기준.** 핀 4장 합 · `S_ref` 기준이 계약값이다. 어댑터가 담당 코드 내부 규약(1장·`S_fin_1`)으로 한 방향 환산한다 |
| `t_wall` 복제 | **GEOM 단독 소유.** `HullOut.t_wall` 로 흐른다. 식의 구현은 `geom.wall_thickness` 하나뿐 |

⚠ 상수 11건은 "**어느 쪽이 맞는지**"가 아니라 "**어느 쪽을 쓸지**"를 정한 것이다.
실측·시험 인쇄로 확정하는 일은 §8 B 에 그대로 남아 있다.

⚠ **담당자에게 보고할 것 — 원본의 잠재 버그.**
`arm_rotor = float(getattr(geo, "arm_rotor", float(dv.arm_rotor)))` 는 파이썬이
getattr 의 기본값을 **먼저** 평가하므로, `geo.arm_rotor` 가 있어도 `dv.arm_rotor` 를
건드려 터진다. ICD0-008 의 `DesignVars` 에는 `arm_rotor` 가 없다(`f_mount` 로 유도).
`[이식 5/5]` 에서 지연 평가로 고쳤다.

### `modules/wght.py` — **스텁 아님** (한 곳 제외)
`_iterate` · `converge` · `mass_props` 는 실제 구현이며 검증된 자산이다.
P0 에서는 ICD §8 C-3 계약 일반화(`strc_of` → `resp_of`)만 했고 상태머신의
분기 조건은 한 줄도 바뀌지 않았다.

| 함수 | 상태 | 없는 것 |
|---|---|---|
| `growth_split(history, resp_parts_of)` | **구현 (P7)** | 상태머신은 안 건드렸다. `resp_parts_of` 를 2회 더 부르므로 **설계점당 +16 %** — `evaluate(dv, split=False)` 로 끌 수 있다 |

⚠ `growth_split` 은 **국소 미분이 아니라 할선**이다. 구조 응답이 `w_fill` 이산화로
계단이라(docs 11-39) 국소 미분은 0 이거나 무한대다. 구간 폭 ±1 % 와 ±5 % 에서
구조 항이 **13 배** 차이 난다 (docs 11-45). 어느 폭을 표준으로 쓸지 미정 [확정 필요]

### `modules/stab.py` — 구현 완료, 계수 TBD (P6)
`SM` · `M_dist`(§4.4 계산값) · `M_ctrl` · `alpha_max`(C5) · g8 · g9 전부 실제 계산.
조종 여유(ΔT)는 런처가 `prop.thrust_max` 로 내서 넘긴다 (STAB→PROP 은 §5 금지).

| 항목 | 상태 | 남은 것 |
|---|---|---|
| `SM`·g8 | 구현 | `SM_min` 은 초기값 1.0 (§3.5) |
| `M_dist` | 구현 | **`Δα`(기준 돌풍 받음각) TBD** — g8·g9 가 여기 달려 있다 |
| `M_ctrl`·g9 | 구현 | `k_ctrl` TBD · **`k_az` 방위각 규약 승인 대기** (X 배치 2√2 전제) |

⚠ **g8·g9 의 실현 가능 창이 매우 좁다.** `M_dist ∝ SM` 이라 안정을 키우면 외란
모멘트가 정비례로 커진다 (docs §11-36). 16점 격자에서 동시 만족이 **1점뿐**이었다.
g8 을 안 건드리고 g9 만 올리는 레버는 `f_mount`(모멘트암)와 `thr_dash`(스로틀 여유)
둘뿐이다. **§5.1 이 지시한 대로 S_fin·x_fin 범위 손질이 스크리닝 최우선이다.**
저장소 표본 설계점은 g1~g9 를 전부 통과하는 점으로 갱신했다
(lambda 8.0 · x_fin 0.55 · f_mount 1.0 → MTOW 1.147 kg).

### `modules/cost.py` — 구현 완료, 단가 TBD (P6)
§5.1 그대로 6항 합산 (모터·배터리·프롭·ESC·프린트·항전). 기준 설계점에서
466,215 KRW — 항전 175,000 · 모터 145,544 · ESC 64,389 · 프롭 32,000 ·
배터리 31,471 · 프린트 17,811.
⚠ 단가 5종이 전부 TBD(§8 B-1)라 **금액의 절대값은 의미가 없다.** 설계점 사이의
상대 비교만 방향이 맞다. C7 가중치가 4.2% 라 §5.1 대로 대표값이면 충분하다.

### `main.py` — 스텁 없음
| 항목 | 상태 | 내용 |
|---|---|---|
| `_close_U_eval` | 구현 | §4.5 순환을 `resp_of` 안에서 닫는다 (구간 유지 regula falsi, 2~5회) |
| `pod_of` 클로저 | 구현 | 포드 치수를 모터 질량에서 만들어 PROP·MISS 에 넘긴다 (§5 "런처가 조립한다") |
| `dims` | 구현 | 밀도 → 부피 → `geom.box_from_volume` 상자 |

---

## 2. 값 미확정 (`constants.py` 의 `# TBD`)

ICD §8 B 에 대응한다. 붙은 숫자는 전부 임시다.

- **B-1 물성·단가** — `rho_pack` `wh_pack` `rho_mot` `k_Rpack` `k_mprop`
  `c_mot_krw` `c_batt_krw` `c_prop_krw` `c_esc_krw` `c_filament_krw`
- **B-2 설계변수 범위** — `d_body` `lambda_body` `S_fin` `x_fin` `AR_fin`
  `d_prop` `pd_prop` `k_E` `k_mot` 범위 전부 (constants 가 아니라 DOE 설정)
- **B-3 임무·요구** — `W_pl` `R_dash_min` `MISSION_PROFILE`(t_seg) `k_trans`
  `t_trans` `c_rate_max` `e_spec` `dt_miss`
- **B-4 안정 판정** — `k_ctrl` `alpha_dot_req` `d_alpha`(Δα)
- **A-2 모터 회귀** — `a_R` `b_R` `c_R` `a_I0` `b_I0` 는 1차 조사값으로 채웠다
  (`data/motor_specs.csv`, `python -m data.fit_motor_regression`). `pm_mot` 는 여전히 TBD
- **A-3 열** — `T_limit` `T_amb` `c_mot` `ar_mot` `Nu_C_*` `Nu_m_*`
- **§8 C-4 잔차 하한** — `resid_floor` `delta_r` (`dt_miss` 확정 후 재측정)
- **배터리 셀** — `U_cell_full` `U_cell_nom` `U_cell_cut`
- **소음 앵커** — `V_tip_ref` `T_ref` `N_ref` `r_obs` (`N_ref` 는 6 dB 가 걸린 미확인 항목)

`# ICD외` 표시 상수 — ICD §3 표에 없는데 코드가 필요로 해서 추가했다. 통합 담당 확인 대상:
`N_rot` `g` `h_miss` `SPL_ref` `r_ref` `r_obs` `V_tip_ref` `T_ref` `N_ref`
`d_clr` `k_esc_margin` `c_filament_krw`
— P2 에서 추가: `f_nose` `k_nose` `lam_fin` `tc_fin` `k_thk` (GEOM 형상 규약) ·
`k_base` `k_int` `k_cal` `k_form` `M_pg_max` (AERO 빌드업) · `Cd_c` `eta_cf` `k_side`
`k_xn` `k_finproj` (크로스플로) · `CN_nose` `k_cp_nose` (Barrowman)
— P3 에서 추가: `k_pitch_margin` (§2 하한 규칙 계수, 원문 1.05→1.25) ·
`B_blade` `r_hub_ratio` `c_over_D` `cl_alpha_2d` `cl_max_2d` `cd0_2d` `k_cd_2d`
(BEMT 블레이드 규약) · BEMT·트림 수치 파라미터 12종

전부 `docs/ICD0-008.md` **§3.9** 에 등재돼 있다.

---

## 3. 지금 깨져 있는 것

- **`R_int_mot` = 0.20 K/W 가 근거 없는 자리표시다.** 덩어리 모델의 낙관성(권선을
  케이스와 등온으로 봄)을 메우려고 넣은 권선→케이스 내부 열저항이다. 이제 이 값이
  모터 크기를 직접 정한다 — 모터 1종 열화상 1회면 잰다 (§8 A-3 에 묶어서).
- **`Nu_C_*`·`Nu_m_*` 가 순항·호버 같은 값이다.** ICD 는 "구간별로 완전히 다르다" 고
  했는데 지금은 유속만 다르게 넣고 있다. 호버는 프롭 후류라 상관식 형태부터 달라야 한다.
- **`c_rate_max`(90C) 와 `k_Rpack`(0.010) 이 서로 모순이다.** 90C 를 허용한다면서
  그 전류에서 팩 전압이 무너진다. 그래서 배터리 활성조건이 `sustain` 으로 나온다.
  둘 다 TBD 이고 **함께** 확정해야 한다 — 실제 팩 1종의 방전 곡선 1회면 둘 다 나온다.
- **g3 는 스크리닝 게이트가 아니라 사이징의 출력이다** (docs 11-44). `size_motor` 가
  트림·팁마하·열을 동시에 만족하는 **최소** 질량을 찾으므로 `k_mot = 1.0` 에서 구속
  조건은 정의상 0 에 붙는다. 7 개 설계점 전부에서 **열이 구속**했고 g3 = +0.000~+0.004
  였다. DOE 보고에서 g2·g3·g4 를 품질 게이트(g5~g9)와 **분리해** 읽어야 한다.
  그리고 `k_mot < 1.0` 은 전량 열로 탈락하므로 **DOE 범위를 [1.0, …] 로 잡아야
  표본이 낭비되지 않는다** [확정 필요]
- **무게 스노우볼의 87.5 % 가 배터리다** (docs 11-45). 모터 9.5 %, 구조 3.0 %.
  구조를 깎아도 MTOW 는 거의 안 줄어든다 — 에너지 쪽이 지렛대다.
- **연속성 판정을 C1 만으로 하면 안 된다** (docs 11-47). `x_fin` 은 C1 이 비트 동일한데
  같은 구간에서 g8 이 0.29 움직인다. C1 무반응을 계단으로 오진하기 쉽다.
  `f_mount` 는 진짜 계단이고 점프 742 mg 이 `w_fill` 계단 743 mg 과 일치한다.
- **설계점당 실행 시간 3.115 s** (`split=False`) / 3.615 s (Ŝ 분해 포함).
  **P7 에서 실측 완료.** DOE 1 만 점이면 단일 프로세스 8.7 시간이라 **병렬 배치가
  사실상 필수**다 (ICD §5 시간단축 ②).
  범인은 **AERO 가 55.8 %** — `aero.CD0` 가 설계점당 **146 만 회**, `_Cf` 가 437 만 회
  불린다. ⓪ 모듈인데 `F_drag`·`CL` 이 MISS 적분과 PROP 트림 안에서 계속 불려서다.
  줄이는 두 길(상수 호이스팅 12 % / ⓪ 표 40 %+)은 docs 11-46 에 적었고, **둘 다
  결과값을 바꾸므로 P7 에서는 하지 않았다** [확정 필요]
- ~~`thr_dash` 근거 없음~~ → **P6 에서 g9 로 교차검증 완료** (docs §11-37).
  0.95 면 g9 가 −0.252 로 불합격이고 0.85 가 거의 최소치다. 임의 상수가 아니라
  g9 에서 도출되는 양임이 확인됐다. 다만 **정식으로는 g9 를 만족하는 최대
  `thr_dash` 를 풀어야** C1 이 최소가 되는데, 그건 ② → ① 되먹임이라 미결이다.
- **§2.2 의 `k_mot` 설명이 실측과 어긋난다** (docs §11-32). "더 빠르다(C3) ↔
  무거워진다(C1)" 인데 실제로는 `k_mot` 1.0→1.4 에서 margin_V 가 **줄어든다**
  (1.1043→1.1029). V_max 의 천장이 모터가 아니라 **프롭의 팁 마하**여서다.
  `k_mot` 이 실제로 사는 것은 **열 여유(g3)** 이고, 지금 g3 은 +0.003 으로 한계에 붙어
  있다. ICD §2.2 개정 필요.
- **C3 가 설계공간을 가르는 것은 팁 마하가 천장일 때뿐이다.** `M_tip_max` 를
  0.70→0.75 로 올리면 천장이 전압으로 넘어가 C3 퍼짐이 0.0465 → 0.0013 으로 무너진다.
  §11-9 의 `M_tip_max` 확정이 C3 의 의미까지 좌우한다.

- ~~TEST 13(c) 실패~~ → **P5.5 에서 해소. 검증 16/16 PASS, CI 초록.**
  `geom.layout` 이 `arm_rotor = r_body + f_mount·b_fin` 을 내면서 알람이 스스로
  꺼졌다 (f_mount 0.4→1.0 에서 arm_rotor 0.101→0.186 m, J_xx 0.97→3.27 g·m²).
  테스트를 고치지 않고 원인을 고쳐서 통과시킨 것이다.
- **`measure_noise.py` 는 응답 질량 중 구조 항만 잰다.** ICD §8 C-4 는 측정 대상을
  전체 응답 질량(구조+모터+배터리)으로 넓히라고 하는데 PROP·MISS 가 스텁이다.
  P4·P5 와 `dt_miss` 확정 후 다시 재야 한다.
- **`README.md` 의 파일 표가 ICD0-007 기준**이다 (`prop_a` `prop_b` `common/srl`).
  담당자 배정 정보가 섞여 있어 임의로 고치지 않았다.

## 4. ICD 승인 대기

`docs/ICD0-008.md` §11 에 개정 15건이 근거와 함께 모여 있고, **전부 승인되어 본문에
반영**됐다 (§11 머리말의 상태표 참조). 그중 아직 **미결**로 남은 항목:

- §11-2 포드 항력 **호출부 배선** — `size_motor` 가 m_mot 을 내므로 이제 포드 치수를
  만들 수 있다. `thrm.motor_geometry(m_mot)` 가 D·L 을 준다 → **P5.6 에서 배선**
- **`pm_mot`(연속 비출력) 값이 지배 변수다** — 아래 §3 참조
- §11-6 질량 분해표 조립의 소유권 (WGHT vs 런처)
- §11-7 **ESC 질량이 ICD 어디에도 없다** — 분해표에서 빠져 있다
- §11-15 BEMT 블레이드 규약 계수 7종 TBD
- ~~§11-16 호버 kv 외삽~~ → **P4 에서 해결.** `solve_point` 에 사이징/평가 두 모드를
  뒀다. dash 가 kv 를 정하고(=1395, 조사 범위 안) 호버는 그 kv 로 부분 스로틀
  평가(38%)를 한다. 실제 쿼드의 호버 스로틀과 맞는다
- **모터 회귀 표본 보충** — 출처 2차 · R 의 상간/상당 확인 · I0 실측치 확보
- **`pd_prop` 상한 미정** — 하한(1.466)만 정했다. 피치가 크면 호버·저속에서
  블레이드가 실속하므로 상한은 P4(호버 열)·P5(호버 에너지)에서 나와야 한다

## 5. 참고

ICD0-007 기준의 GEOM·AERO·PROP-A/B·STRC·STAB·COST 구현은 커밋 `d801929` 에 남아
있다. P2·P6 에서 수식을 이식할 때 참고한다 (설계변수와 g 번호가 달라 그대로는 못 쓴다).

## 6. DOE 배치층 (`doe/`)

P7 다음 단계(ICD §8 D-2 스크리닝 DOE)의 **배관만** 깔았다. 사이징 경로는 한 줄도
바뀌지 않았다 — `doe/worker.py` 가 `main.evaluate` 를 그대로 부른다.

| 항목 | 상태 | 남은 것 |
|---|---|---|
| `doe/space.py` 상자 정의 | 구현 | **`screen` 상자의 6 축이 비어 있다** (아래) |
| `doe/sample.py` LHS | 구현 | 표준 라이브러리로 직접 깔았다 (scipy 미사용) |
| `doe/row.py` 평면화 | 구현 | `Result` 를 아는 유일한 곳 |
| `doe/run.py` 배치·재개·매니페스트 | 구현 | — |
| `doe/report.py` 분석 | **일부** | EC 가중합 점수 미구현 · 민감도는 4분위 합격률뿐 |
| `doe/gui.py` 런처 창 | 구현 | tkinter. 계산은 없고 `doe.run` 을 자식 프로세스로 띄운다 |

### 비어 있는 것

- **`screen` 상자의 6 축이 `None`** — `d_body` `lambda_body` `S_fin` `x_fin_ratio`
  `AR_fin` `pd_prop`(상한). ICD §8 B-2 미확정이라 값을 지어넣지 않았고, 그 상태로
  실행하면 런처가 거부한다. `--set` 으로 넣어야 돈다. ICD §2 가 범위를 준 5 축
  (`f_mount` `n_design` `d_prop` `k_E` `k_mot`)만 채워져 있다.
- **EC 가중합 점수(§6.2) 미구현** — C1~C8 은 단위가 제각각이라 정규화가 필요한데
  기준이 없다. 표본 최소–최대로 할지 요구조건 기준으로 할지에 따라 순위가 뒤집힌다.
  설계점 하나로는 원리적으로 못 내는 값이라 `evaluate` 가 아니라 분석 쪽 소관이다.
  [확정 필요]
- **정식 민감도 미구현** — 지금은 축별 4분위 합격률뿐이다. 주효과·상호작용 분해는
  스크리닝 표본이 쌓인 뒤에 붙인다.

### 확정 필요

- **탐색 박스를 `constants.py` 밖에 뒀다.** CLAUDE.md 규칙 2 와 부딪히는 지점이다.
  박스는 물리 상수가 아니라 배치마다 바뀌는 실행 설정이고, 어느 상자에서 나온
  로그인지가 매니페스트에 남아야 의미가 있다고 판단했다. 반대면 옮긴다.
- **`space.l_body_of` 가 `geom.hull` 의 전장 식을 두 번째로 적고 있다.**
  `x_fin_ratio` 를 절대 위치로 바꾸려면 `DesignVars` 를 만들기 전에 전장이 필요한데
  `geom.hull(dv)` 는 그 `DesignVars` 를 받는다. `geom` 에 접근자를 두는 편이 맞다.
- **`n_ser` 수준마다 시드를 갈라 서로 다른 표본을 뽑는다.** 같은 형상에 셀 수만
  바꾸는 쪽이 비교는 깔끔하지만 형상 공간을 덜 훑는다.
- **복제 비율 기본 1 %** — 순수성 감시 비용이다. 배치가 커지면 낮춰도 된다.
