...
beta = 0.5                # 완화계수. 갱신식과 Ŝ 환산식 두 줄에만 등장시킨다 (WGHT §3)
k_init = 1.4
N_iter_max = 50

# 수렴 '상태' 판정용 — 발산·리밋사이클을 '해석 실패'와 구분하기 위한 값들 (WGHT §3)
resid_floor = 1e-6        # ★ MTOW 대비 상대 하한. resid가 이 아래로 내려가면 r̂은 수치
                          #   노이즈만 재고, 노이즈는 방향이 없어 Ŝ가 1 근처에도 이상에도 찍힌다.
                          #   실물 STRC 없이는 원리적으로 못 정한다 → k_sl_* 실측 후 재측정 필수
delta_r = 0.05            # ★ 진동 발산(r̂ ≤ -(1+delta_r))과 리밋사이클(r̂ ≈ -1)의 구분 여유
